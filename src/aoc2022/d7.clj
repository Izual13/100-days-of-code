...
(map parse-input (filter not-empty test-input))

(defn parse-file [f]
  (let [[_ size name] (re-matches #"(\d*) (.*)" f)]
    {:s (Long/parseLong size) :n name}))

(parse-file "14848514 b.txt")
...
(parse-files `("dir a" "14848514 b.txt" "8504156 c.dat" "dir d"))
...
(defn calc-directories
  ...
  ([d s] 
   ...
   (let [new-d (get-in d s)
         _ (println (keys (dissoc new-d :files)))
     ...
     {:s (+ size (reduce + (map :s children))) :c (vec children) :d s})))
